...
class Time:
    ...
    _steady_state: NDArray[np.bool_] | None = field(default=None, init=False)

    # Cache dictionaries
    _totim_dict: dict = field(init=False, factory=dict)
    _pertim_dict: dict = field(init=False, factory=dict)
    _datetime_dict: dict = field(init=False, factory=dict)
    _str_format: str = field(init=False, default="%Y-%m-%dt%H:%M:%S")
    ...
    start_datetime : various objects
        ...
        Time.parse_datetime documentation for a list
    ...
    def __init__(
        ...
        if len(self._perlen) != len(self._nstp) or len(self._perlen) != len(self._tsmult):
            raise ValueError("perlen, nstp and tsmult must have the same length")
        if steady_state is not None:
            self._steady_state = np.atleast_1d(_maybe_collect(steady_state)).astype(bool)
            if len(self._steady_state) != len(self._perlen):
                raise ValueError("perlen, nstp, tsmult and steady_state must have the same length")
    ...
    def start_datetime(self, value: str | datetime | np.datetime64 | pd.Timestamp | None):
        self._start_datetime = Time.parse_datetime(value)
    ...
    def get_datetime_string(
        ...
        dt = Time.parse_datetime(datetime_obj)
    ...
    def _get_datetime_string_format(str_datetime: str) -> str:
        ...
        str_datetime : str
            ...
            Time.parse_datetime documentation for supported
            formats
    ...
    def parse_datetime(
        ...
        if datetime_obj is None:
            ...
            str_rep = Time._get_datetime_string_format(datetime_obj)
    ...
    def get_elapsed_time(self, kper: int, kstp: int | None = None, start: bool = False) -> float:
        """
    ...
    def get_datetime(self, kper: int, kstp: int | None = None, start: bool = False) -> datetime:
        """
        ...
        if self.time_units == "unknown":
            raise AssertionError("time units must be set in order to calculate datetime")
    ...
    def intersect(
        ...
        if datetime_obj is not None:
            datetime_obj = Time.parse_datetime(datetime_obj)
            ...
            raise AssertionError("A date-time representation or totim needs to be provided")
        ...
        if totim > self.totim[-1] or totim <= 0:
            ...
            if datetime_obj is None:
                msg = (
                    f"supplied totim {totim} is outside of model's time domain 0 - {self.totim[-1]}"
                )
            else:
    ...
    def from_perioddata(
        ...
        """
        Instantiate a Time class from a TDIS perioddata array.
    ...
    @classmethod
    def from_headers(cls, headers: np.recarray) -> "Time":
        """
        Instantiate a Time class from a head or budget file header array.
    ...
    def reverse(self) -> "Time":
        ...
        Returns
        -------
            Time object with reversed order of stress periods and time steps.
        """
        return Time(
            self.perlen[::-1],
            self.nstp[::-1],
