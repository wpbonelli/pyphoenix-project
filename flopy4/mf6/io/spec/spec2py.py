...
class Spec2Py:
    """
    Generate Python source code from a MODFLOW 6
    ...
    def __init__(
        ...
        with open(tomlfspec, "r") as fh:
            tin = toml.load(fh)
        with open(f"{PROJ_ROOT}/spec/package.template") as f:
...
class Toml2IModel:
    ...
    def __init__(
        ...
        with open(f"{PROJ_ROOT}/spec/model.template") as f:
            tout = Template(f.read())
...
if __name__ == "__main__":
    ...
    toml_d = {}
    for t in tomls:
        converter = Spec2Py(t, str(outdir))
        if verbose:
            converter.warn()
