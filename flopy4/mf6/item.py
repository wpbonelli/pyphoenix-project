...
def _cellid_field(cls: type) -> Any | None:
    return next(
        (f for f in record_fields(cls).values() if (f.json_schema_extra or {}).get("cellid")), None
    )


def _has_aux_field(cls: type) -> bool:
    return "aux" in record_fields(cls)


def _has_boundname_field(cls: type) -> bool:
    return "boundname" in record_fields(cls)
...
def _is_item_union(annotation: Any) -> "tuple[type[Item], ...] | None":
    ...
    string. Not needed here: by the time `record_fields()` has run,
...
def construct_item(item_cls: type, values) -> "Item":
    ...
    """
    fields = list(record_fields(item_cls).items())
    tuple_idx = next(
        (
            i
